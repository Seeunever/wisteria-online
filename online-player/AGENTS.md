...
- 正式部署使用代码目录外独立 `STATE_DIR`，不得复制本地测试账号或进度。正式模式默认关闭测试辅助并使用 HTTPS；用户最新明确要求线上应邪也提供补完按钮，允许仅对该本设置 `YINGXIE_TEST_ASSIST=1`，不自动扩展给紫藤或新本。配置与恢复见 `DEPLOYMENT.md`，检查入口 `check-deployment.mjs`。现有服务器还有其他服务，替换仅限 Wisteria 网站。
