...
function fail() {
  process.stderr.write('{"code":"PACK_INSTALL_REJECTED","status":"blocked"}\n');
  process.exitCode = 2;
}
...
try {
  ...
  for (const [sourceId, source] of Object.entries(bundle.sources)) {
    if (!/^src_[0-9a-f]{8,64}$/.test(sourceId) || source?.sourceId !== sourceId) {
      throw new Error('SOURCE_REJECTED');
    }
    ...
    const objectPath = path.join(objectsDirectory, `${sourceId}.source`);
    copyFileSync(sourcePath, objectPath, constants.COPYFILE_EXCL);
    chmodSync(objectPath, 0o600);
    if (sha256(objectPath) !== normalizedDigest(source.sha256)) throw new Error('COPY_MISMATCH');
  }
